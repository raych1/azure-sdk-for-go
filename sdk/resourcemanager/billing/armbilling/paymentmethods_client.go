...
// BeginDeleteAtBillingProfile - Deletes a payment method link and removes the payment method from a billing profile. The
...
//   - options - PaymentMethodsClientBeginDeleteAtBillingProfileOptions contains the optional parameters for the PaymentMethodsClient.BeginDeleteAtBillingProfile
//     method.
func (client *PaymentMethodsClient) BeginDeleteAtBillingProfile(ctx context.Context, billingAccountName string, billingProfileName string, paymentMethodName string, options *PaymentMethodsClientBeginDeleteAtBillingProfileOptions) (*runtime.Poller[PaymentMethodsClientDeleteAtBillingProfileResponse], error) {
	if options == nil || options.ResumeToken == "" {
		resp, err := client.deleteAtBillingProfile(ctx, billingAccountName, billingProfileName, paymentMethodName, options)
		if err != nil {
			return nil, err
		}
		poller, err := runtime.NewPoller(resp, client.internal.Pipeline(), &runtime.NewPollerOptions[PaymentMethodsClientDeleteAtBillingProfileResponse]{
			Tracer: client.internal.Tracer(),
		})
		return poller, err
	} else {
		return runtime.NewPollerFromResumeToken(options.ResumeToken, client.internal.Pipeline(), &runtime.NewPollerFromResumeTokenOptions[PaymentMethodsClientDeleteAtBillingProfileResponse]{
			Tracer: client.internal.Tracer(),
		})
	}
}

// DeleteAtBillingProfile - Deletes a payment method link and removes the payment method from a billing profile. The operation
// is supported only for billing accounts with agreement type Microsoft Customer Agreement.
// If the operation fails it returns an *azcore.ResponseError type.
//
// Generated from API version 2024-04-01
func (client *PaymentMethodsClient) deleteAtBillingProfile(ctx context.Context, billingAccountName string, billingProfileName string, paymentMethodName string, options *PaymentMethodsClientBeginDeleteAtBillingProfileOptions) (*http.Response, error) {
	var err error
	const operationName = "PaymentMethodsClient.BeginDeleteAtBillingProfile"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.deleteAtBillingProfileCreateRequest(ctx, billingAccountName, billingProfileName, paymentMethodName, options)
	if err != nil {
		return nil, err
	}
...
// deleteAtBillingProfileCreateRequest creates the DeleteAtBillingProfile request.
func (client *PaymentMethodsClient) deleteAtBillingProfileCreateRequest(ctx context.Context, billingAccountName string, billingProfileName string, paymentMethodName string, options *PaymentMethodsClientBeginDeleteAtBillingProfileOptions) (*policy.Request, error) {
